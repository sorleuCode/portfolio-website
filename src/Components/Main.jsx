        ...
        <div>
          ...
          <ul className='w-full flex gap-3 mt-5 lg:hidden'>
            {socials.map(({id, child, href, style, name, download}) => (
              <li key={id} className={`hover:scale-125 duration-300`}><a target='_blank' className={` text-cyan-400 ${style} text-lg sm:text-2xl md:text-3xl`} title={name} href={href} download={download}>{child}</a></li>
            ))}
          </ul>
